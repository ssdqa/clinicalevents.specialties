...
#' Concordance: Clinical Events and Specialties -- Output
...
cnc_sp_output <- function(cnc_sp_process_output,
  ...
  if(multi_or_single_site=='single'&anomaly_or_exploratory=='exploratory'){
    # over time
    if(time){
      conc_output_plot <- cnc_sp_ss_exp_at(data_tbl=conc_output_pp,
                                           facet=facet_vars)
      ...
      conc_output_plot <- cnc_sp_ss_exp_nt(data_tbl=conc_output_pp,
                                           facet=facet_vars,
    ...
    if(time){
      ...
      conc_output_plot<-cnc_sp_ss_anom_at(process_output=conc_output_pp,
      ...
      # not over time
      conc_output_plot <- cnc_sp_ss_anom_nt(data_tbl=conc_output_pp,
                                            facet = facet_vars)
    }
    ...
    if(time){
      ...
      conc_output_plot <- cnc_sp_ms_exp_at(data_tbl=conc_output_pp,
                                           facet=facet_vars)
    }else{
      # not over time
    ...
    if(time){
      # over time
      conc_output_plot <- cnc_sp_ms_anom_at(process_output=conc_output_pp,
